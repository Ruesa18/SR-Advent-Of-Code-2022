...
function partOne() {
    try {
        ...
        const count = findCompletelyOverlappingSum(elfGroups);
...
function findCompletelyOverlappingSum(elfGroups) {
    let collisions = [];
    ...
    return collisions.reduce((partialSum, currentContained) => {
        return currentContained.fullyContained ? partialSum + 1 : partialSum;
    }, 0);
}
